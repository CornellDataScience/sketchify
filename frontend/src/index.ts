...
ipcMain.on('open-directory-dialog', (event) => {
  dialog.showOpenDialog(mainWindow, {
      ...
      event.reply("open-image-response", result.filePaths[0]);
...
ipcMain.on('open-directory-sketch', (event) => {
  dialog.showOpenDialog(mainWindow, {
      ...
      console.log(result.filePaths);
      event.reply("open-sketch-response", result.filePaths[0]);
...
ipcMain.on('check-image-similarity',(event, imgPath, sketchPath) => {
    ...
    exec(`python ../ml/main.py ${imgPath} ${sketchPath}`, (error, stdout, stderr) => {
      if (error) {
        console.error(`exec error: ${error}`);
        event.reply("python-script-response", `Error: ${error.message}`);
        return;
